...
public class WC15 {
	...
	public static String centralThree(String letters) {
		return letters.substring(letters.length() / DIVISIOR - 1, letters.length() / DIVISIOR + DIVISIOR);
	}
	...
	/**
	 * Returns an array with the specified number of rows and columns containing the
	 * characters from str in row-major order. If str.length() is greater than rows
	 * * cols, extra characters are ignored. If str.length() is less than rows *
	 * cols, the remaining elements in the returned array contain null.
	 * 
	...
	/**
	 * Returns an array with the specified number of rows and columns that contains
	 * the elements of vals in the order specified below. Elements from vals are
	 * placed in the array by moving down the first column, up the second column and
	 * so on.
	 * 
	 * <pre>
	...
	/**
	 * Returns a smaller array containing the elements in the specified range of the
	 * mat.
	 * 
	 * For example,
	 * 
	 ...
	 */
	public static int[][] crop2D(int[][] mat, int startRow, int startCol, int endRow, int endCol) {
		// TODO: Finish
